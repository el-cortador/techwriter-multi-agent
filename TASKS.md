...
### - [x] 0.4. Безопасные дефолты в install-скриптах
...
### - [x] 1.1. Единый источник схемы БД
...
- **1.1.** `SCHEMA_SQL` в `hermes/app/telemetry.py` и `dashboard-api/app/main.py` не были
  идентичны, как утверждает SPEC.md: в `telemetry.py` были 5 дополнительных
  `CREATE INDEX IF NOT EXISTS`, которых не было в `dashboard-api/app/main.py`. За основу
  `db/schema.sql` взята версия из `telemetry.py` (с индексами) как более полная;
  `dashboard-api` их не имел с самого начала — это баг в исходном коде, теперь исправлен.
- **1.1.** Реальный локальный `.env` репозитория (не в git) не содержал `DASHBOARD_API_KEY`/
...
- **1.3.** `get_pool()` в `hermes/app/db.py` и `dashboard-api/app/db.py` — ленивый синглтон
  (создаётся при первом обращении, а не безусловно на импорте модуля), чтобы сохранить
  поведение `telemetry.is_enabled()`: при пустом `DATABASE_URL` пул не пытается открыть
  соединение вообще, а не падает/ретраит в фоне. Проверено нагрузочно на реальном стеке:
